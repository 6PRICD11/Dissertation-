...
<html>
<head>
    <title>OpenStreetMap Plot</title>
    ...
    <script>
        ...
        locations.forEach(function(location) {
            L.marker([location.latitude, location.longitude]).addTo(map)
                .bindPopup("<b>IP Address:</b> " + location.address);
        });
    </script>
</body>
</html>
...
local function to_json(tbl)
    ...
    for _, entry in ipairs(tbl) do
        table.insert(items, string.format(
            '{ "address": "%s", "latitude": %.6f, "longitude": %.6f }',
...
-- Write HTML file
local file = io.open("OSMmap.html", "w")
if file then
    file:write(html_template:format(to_json(addresses)))
    file:close()
    print("HTML file 'OSMmap.html' created. Open it in a browser to view the map.")
else
    print("Error: Unable to create 'OSMmap.html'")
end
